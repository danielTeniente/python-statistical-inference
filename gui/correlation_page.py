...
def render_correlation_page():
    ...
    # Unique identifier to clear results if variables or method change
    analysis_id = f"{selected_x}_{selected_y}_{selected_method}"
    ...
    if st.button(f"Calculate {selected_method}", key="btn_calc"):
        with st.spinner(f"Calculating {selected_method} correlation..."):
            if selected_method == "Pearson":
                st.session_state.corr_results["calc"] = perform_pearson_correlation(df, selected_x, selected_y)
            elif selected_method == "Spearman":
                st.session_state.corr_results["calc"] = perform_spearman_correlation(df, selected_x, selected_y)
            elif selected_method == "Kendall":
                st.session_state.corr_results["calc"] = perform_kendall_correlation(df, selected_x, selected_y)
    ...
    if "calc" in st.session_state.corr_results:
        ...
        if selected_method == "Pearson":
            metric_name, ci_name = "Pearson's r", "95% Confidence Interval"
        elif selected_method == "Spearman":
            metric_name, ci_name = "Spearman's rho", "95% Bootstrap CI"
        else:
            metric_name, ci_name = "Kendall's tau", "95% Bootstrap CI"
